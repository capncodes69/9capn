...
- **The stream is NOT SSE.** `Accept: text/event-stream` is answered with `Content-Type: text/plain` + chunked **AI SDK v4 data-stream** frames (`f:`/`0:`/`d:`/`e:`/`z:`…). Parse chunked framing + prefixes, never `data:`. Text-only by design: tool frames (`9:`/`a:`/`b:`) are dropped. `d:` carries real `{promptTokens, completionTokens}` — that is what the usage ledger records.
...
- **No quota or credit API either.** `GET /me` returns identity only (`user_id`, `email`, `username`, `teams[]`), so `services/usage/camber.js` reports *which account and agent* a connection is bound to and deliberately fabricates no progress bar; compute jobs are metered on Camber's own dashboard.
...
**The agent is a per-connection setting** (`providerSpecificData.camberAgent`, default `nova.cli`, stored without the leading `@` the wire wants), because a model cannot be pinned the same way: the agent determines the tool/skill surface and the model is chosen per request. `AddApiKeyModal` asks for it next to the key, which is why the API-key path is not just "paste and go".

**When Camber changes, re-probe in this order:** (1) the `/chat` frame prefixes and the `d:` usage field names; (2) the accepted `effort` values — send a bogus one and read the `oneof` error, which lists the legal set (that is how `low|medium|high` was confirmed); (3) whether a `/models` or quota path has appeared (re-run the probe list in the `camberAuth.js` header); (4) `/auth/initiate` + `/auth/poll` status codes and the base64 blob shape — a change here breaks login **silently**, which is why `decodeCamberLoginToken()` validates the shape and 502s rather than saving a blank connection.
